...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const { rows } = await pool.query(
      `SELECT style_id FROM nk_club_styles WHERE club_id = $1 ORDER BY style_id`,
      [req.params.id]
    );
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      await client.query("BEGIN");
      await client.query(`DELETE FROM nk_club_styles WHERE club_id = $1`, [
        req.params.id,
      ]);
      for (const styleId of styleIds) {
        await client.query(
          `INSERT INTO nk_club_styles (club_id, style_id) VALUES ($1, $2)`,
